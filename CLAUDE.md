...
**generators**: yield/generator_return opcodes must `return` from runLoop, not `continue` (continue re-enters the loop instead of exiting to the caller). generator exception isolation uses `handler_floor` (same pattern as fibers) - without it, throw inside a generator's nested runLoop finds the caller's catch handlers, executes catch/post-catch/halt inside the generator context, and the outer runLoop resumes with IP past end of bytecode. `pending_exception` carries the exception back to the caller scope for correct dispatch
...
- `zphp build <file>` - compile PHP to bytecode and write a `.zphpc` file. skips parse/compile on subsequent runs
- `zphp build --compile <file>` - produce a standalone executable. copies the zphp binary and appends the compiled bytecode with a magic trailer. the resulting binary has the full runtime, all linked C libs, and the bytecode in one file - no zphp installation needed to run it
...
two modes, mirroring bun's build model:

**`zphp build <file>`** - AOT bytecode compilation. compiles PHP source through the full pipeline (lexer -> parser -> compiler) and serializes the resulting bytecode chunks to a `.zphpc` file. running `zphp run file.zphpc` loads the pre-compiled bytecode directly, skipping parse/compile entirely. the serialization format needs to capture: opcodes, constant pools, string tables, function definitions, class definitions.

**`zphp build --compile <file>`** - standalone executable. does everything `zphp build` does, then copies the zphp binary itself and appends the compiled bytecode to the end of it with a magic marker/offset trailer (last N bytes). at startup, `main.zig` checks for an embedded payload before parsing CLI args - if found, loads and executes the embedded bytecode directly. the resulting binary is fully self-contained: VM, all linked C libs (pcre2, sqlite3, zlib, mysql, pgsql), and the application bytecode in a single file. no zphp installation required on the target machine.

implementation: layer 4 tool (see ARCHITECTURE.md layering model). needs a bytecode serialization format and a self-exe detection path in main.zig.

## roadmap

1. `zphp build` - bytecode compilation and standalone executables (see "zphp build" section)
2. union type enforcement - `int|string` runtime checking at function boundaries. currently parsed but ignored
3. match expression exhaustiveness - `match` without default should throw `UnhandledMatchError`
4. complex string interpolation - `"${expr}"` and `"{$obj->prop}"` edge cases
5. performance baseline - benchmark against PHP 8.4 on compute-heavy tasks
