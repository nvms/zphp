...
class JobQueue {
    ...
    public function processNext(): ?Job {
        ...
        if ($job->output !== null) {
            $this->stats['total_output_bytes'] += strlen($job->output);
        }
    ...
    public function recentFirst(): array {
        ...
        $result = [];
        for ($this->completed->rewind(); $this->completed->valid(); $this->completed->next()) {
            $result[] = $this->completed->current()->name;
        }
