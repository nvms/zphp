...
pub const VM = struct {
    ...
    fn initConstants(c: *std.StringHashMapUnmanaged(Value), a: Allocator) !void {
        ...
        try c.put(a, "EXTR_PREFIX_ALL", .{ .int = 2 });
        ...
        try c.put(a, "GLOB_NOESCAPE", .{ .int = 64 });
        try c.put(a, "GLOB_BRACE", .{ .int = 128 });
        try c.put(a, "GLOB_ONLYDIR", .{ .int = 8192 });
        try c.put(a, "GLOB_ERR", .{ .int = 4 });
        try c.put(a, "GLOB_AVAILABLE_FLAGS", .{ .int = 8 | 32 | 16 | 64 | 128 | 8192 | 4 });
